...
export function CartPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-[1280px] mx-auto px-margin-desktop py-2xl text-center">
          ...
          <div className="space-y-md">
            {items.map((item) => (
              <div key={item.producto_id} className="bg-surface-container rounded-lg p-md flex flex-col sm:flex-row items-start sm:items-center gap-lg border border-outline-variant/30 hover:border-primary/50 transition-all duration-300">
                <div className="w-full sm:w-24 h-48 sm:h-24 rounded overflow-hidden shrink-0">
                  <img
                    src={getProductImage(item.producto_id, item.producto_id)}
                    alt={item.nombre}
                ...
                <div className="flex-grow">
                  ...
                  <p className="font-body-md text-body-md text-on-surface-variant">${item.precio.toFixed(2)} c/u</p>
                ...
                <div className="sm:text-right w-full sm:w-auto pt-md sm:pt-0 border-t sm:border-t-0 border-outline-variant/20">
                  <span className="font-headline-md text-headline-md text-on-surface">${(item.precio * item.cantidad).toFixed(2)}</span>
                </div>
              </div>
            ))}
          </div>
